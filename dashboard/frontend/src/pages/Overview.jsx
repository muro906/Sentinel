...
function StatCard({ icon: Icon, label, value, sub, iconClass, onClick }) {
  return (
    <div
      onClick={onClick}
      className={cn(
        "bg-theme-surface border border-theme rounded-lg p-4 flex items-start gap-3 transition-all",
        onClick && "cursor-pointer hover:border-blue-500/40 hover:bg-theme-raised"
      )}
    >
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard
          icon={ShieldAlert} label="Total Alerts" value={total}
          iconClass="text-blue-400"
          onClick={() => navigate('/alerts')}
        />
        <StatCard
          icon={AlertTriangle} label="Critical" value={critical}
          iconClass="text-red-400"
          sub={critical > 0 ? 'Requires immediate attention' : 'No critical alerts'}
          onClick={() => navigate('/alerts?priority=critical')}
        />
        <StatCard
          icon={Clock} label="Awaiting Action" value={needsAction}
          iconClass="text-amber-400"
          sub="received + triaged + pending"
          onClick={() => navigate('/alerts')}
        />
        <StatCard
          icon={CheckCircle} label="Resolved" value={resolved}
          ...
          <div className="space-y-2.5">
            {PRIORITY_ORDER.map(p => {
              ...
              return (
                <div
                  key={p}
                  onClick={() => navigate(`/alerts?priority=${p}`)}
        ...
        <div className="bg-theme-surface border border-theme rounded-lg p-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {STATUS_ORDER.map(s => {
              ...
              const dest = HISTORY_STATUSES.has(s) ? '/alerts?tab=history' : '/alerts'
              return (
                <div
                  key={s}
                  onClick={() => navigate(dest)}
                  className="flex items-center justify-between bg-theme-raised rounded px-2.5 py-1.5 cursor-pointer hover:bg-theme-raised/80 hover:border-blue-500/30 border border-transparent transition-all"
                >
      ...
      <div className="bg-theme-surface border border-theme rounded-lg overflow-hidden">
        ...
        <table className="w-full text-xs">
          <tbody>
            {recent.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-8 text-center text-theme-muted text-xs">No alerts yet</td>
              </tr>
            ) : recent.map(a => (
              <tr
                key={a.alert_id}
