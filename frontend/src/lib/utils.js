...
export function priorityColor(p){
    return({
        ...
        low: 'text-slate-400 border-slate-800 bg-slate-950'
    })[p.toLowerCase()] ?? 'text-slate-400 border-slate-700 bg-slate-800'
}
...
export function statusColor(s){
    return({
        received: 'text-slate-300 bg-slate-800',
        triaged: 'text-blue-300 bg-slate-950',
        ...
        closed: 'text-slate-500 bg-slate-900'
    })[s.toLowerCase()] ?? 'text-slate-400 bg-slate-800'
}
